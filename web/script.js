...
let viewerIndex = -1;
let viewerZoom = 1;
function openGalleryViewer(id) { if (gallerySelectMode) { if (_dragSuppressClick) { _dragSuppressClick = false; return; } toggleGallerySelected(id); return; } viewerIndex = galleryState.images.findIndex(i => i.id === id); if (viewerIndex < 0) return; viewerZoom = 1; showViewerImage(); }
...
function toggleViewerFav() {
    ...
    async function writeClipboardPath(path) {
        // ponytail: file URI via wl-copy — Klipper/Dolphin get a real file entry
        const r = await fetch("/api/clipboard?uri=1", { method: "POST", body: path });
        if (!r.ok) {
            const error = await r.text();
            console.error("Native clipboard API failed:", r.status, error);
            throw new Error(`Native clipboard API failed: ${r.status}`);
        }
    }
    ...
    async function copyToClipboard(url, filename) {
        try {
            await writeClipboardPath(clipboardRelPath(url, filename));
    ...
    async function copyViewerImage() {
        ...
        try {
            ...
            if (!blob) { showToast("Image is not ready yet — try again", { warn: true, icon: WARN_ICON }); return; }
            showToast("Copying...", { icon: COPY_ICON });
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Escape') {
            ...
            // block the webview's native copy so it can't race/overwrite our clipboard write
            e.preventDefault();
            e.stopPropagation();
            copyViewerImage();
        }
